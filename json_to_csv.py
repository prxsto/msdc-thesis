...
def parse_building(bldg, filename):
    ...
    for y in list(range(1, 51)):
        ...
        data['Total GWP Linear (kg CO2e / ft2) {} year'.format(y)
             ] = opl_year_x + accl + emb
        data['Op GWP Linear (kg CO2e / ft2) {} year'.format(y)
             ] = opl_year_x + accl

        data['Total GWP non-linear 2% (kg CO2e / ft2) {} year'.format(y)
             ] = op2_year_x + acc2 + emb
        data['Op GWP non-linear 2% (kg CO2e / ft2) {} year'.format(y)
             ] = op2_year_x + acc2

        data['Total GWP non-linear 3% (kg CO2e / ft2) {} year'.format(y)
             ] = op3_year_x + acc3 + emb
        data['Op GWP non-linear 3% (kg CO2e / ft2) {} year'.format(y)
             ] = op3_year_x + acc3

        data['Total GWP non-linear 5% (kg CO2e / ft2) {} year'.format(y)
             ] = op5_year_x + acc5 + emb
        data['Op GWP non-linear 5% (kg CO2e / ft2) {} year'.format(y)
             ] = op5_year_x + acc5

        # non linear models - - - - - - -

        if city_gwp2 - delta2 >= 0.:
            # X% of initial city value, 5% gets to 0 (GWP/kWh) in 20y (Paris)
            city_gwp2 -= delta2
        else:
            city_gwp2 = 0.
...
def plot_licecyle_models(data, keys=None):
    ...
    for k in keys:
        ...
        for y in years:
            ...
            lin.append(
                data[k]['Total GWP Linear (kg CO2e / ft2) {} year'.format(y)])
            opl.append(
                data[k]['Op GWP Linear (kg CO2e / ft2) {} year'.format(y)])

            nl2.append(
                data[k]['Total GWP non-linear 2% (kg CO2e / ft2) {} year'.format(y)])
            onl2.append(
                data[k]['Op GWP non-linear 2% (kg CO2e / ft2) {} year'.format(y)])

            nl3.append(
                data[k]['Total GWP non-linear 3% (kg CO2e / ft2) {} year'.format(y)])
            onl3.append(
                data[k]['Op GWP non-linear 3% (kg CO2e / ft2) {} year'.format(y)])

            nl5.append(
                data[k]['Total GWP non-linear 5% (kg CO2e / ft2) {} year'.format(y)])
            onl5.append(
                data[k]['Op GWP non-linear 5% (kg CO2e / ft2) {} year'.format(y)])

        fig.add_trace(go.Scatter(x=years, y=opl,
                                 name='Op Linear - {}'.format(k)))
        fig.add_trace(go.Scatter(x=years, y=onl2,
                                 name='Op Non linear 2% - {}'.format(k)))
        fig.add_trace(go.Scatter(x=years, y=onl3,
                                 name='Op Non linear 3% - {}'.format(k)))
        fig.add_trace(go.Scatter(x=years, y=onl5,
                                 name='Op Non linear 5% - {}'.format(k)))

        fig.add_trace(go.Scatter(x=years, y=lin,
                                 name='Total linear 2% - {}'.format(k)))
        fig.add_trace(go.Scatter(x=years, y=nl2,
                                 name='Total Non linear 2% - {}'.format(k)))
        fig.add_trace(go.Scatter(x=years, y=nl3,
                                 name='Total Non linear 3% - {}'.format(k)))
        fig.add_trace(go.Scatter(x=years, y=nl5,
                                 name='Total Non linear 5% - {}'.format(k)))

        fig.add_trace(go.Scatter(x=years, y=emb,
                                 name='Embodied - {}'.format(k)))
...
if __name__ == '__main__':
    ...
    frame.to_csv(os.path.join(studio2021.TEMP, 'assemblies_data.csv'))
